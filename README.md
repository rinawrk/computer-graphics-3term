...
### Результат
В результате лабораторной работы подготовлена база для дальнейших заданий по OpenGL: проект настроен, окно и контекст создаются корректно.
...
Данные вершин размещаются в буферах на стороне видеокарты, конфигурация вершинных атрибутов сохраняется в **VAO**, а обработка вершин и цвета переносится в **GLSL-шейдеры**. Дополнительно в работу вводятся `uniform`-переменные и простая анимация цвета объекта во времени. 
...
- шейдеры вынесены в отдельные файлы `.vert` и `.frag`; это упрощает структуру проекта и дальнейшее расширение; 
...
Они заранее загружаются в память видеокарты, а их обработка выполняется через программируемые шейдеры. Такой подход соответствует современному OpenGL и становится базой для всех последующих 3D-лабораторных работ. 

### Результат
В результате лабораторной работы реализован рендер фигуры средствами современного OpenGL: используются буферы, шейдерные программы и `uniform`-переменные.
...
### Основные идеи
Передать общую форму конструкции по референсу и правильно разделить её на подвижные части для последующей анимации и использования в OpenGL. 

### Что реализовано
- создана 3D-модель промышленного робота **М20Ц** в **Blender**;
...
### Результат
В результате лабораторной работы подготовлена собственная составная 3D-модель робота **М20Ц**, разбитая на подвижные части.  
Эта структура служит основой для последующего импорта модели в OpenGL, настройки освещения и реализации движения в следующих лабораторных работах.
...
![Вид на соединение](assets/blender_model_M20C/Images/Lab3_joint.PNG)
